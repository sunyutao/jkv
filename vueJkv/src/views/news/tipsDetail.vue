  ...
  <div>
    ...
    <div class="bread-crumb-container">
      <div class="bread-crumb">
        ...
        》
        <router-link target="_blank" to="/tipsList">政策速递</router-link>
        》
        <span class="art-title">{{detail.tipTitle}}</span>
      </div>
    </div>
    <div class="news-view">
      <div class="content">
        <div class="left">
          <h1 class="art-type">{{detail.tipTypeNm}}<span>{{formatDate(detail.tipPubDate,"-")}}</span></h1>
          <h2 class="art-title h2-line">{{detail.tipTitle}}</h2>
        ...
        <div class="right">
          ...
          <ul class="art-list" v-for="item in detalList">
            <li v-if="item.tipId !=$route.params.id">
              <span></span>
              <router-link target="_blank" :to='{path:"/tipsDetail/"+item.tipId}' @click="getDetail()">{{item.tipTitle}}</router-link>
            </li>
          </ul>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail: function () {
        var _this = this
        var str = _this.$qs.stringify({tipId: _this.$route.params.id})
        _this.$ajax.post('/cnews/tips/inter/gettipsone.do', str).then(function (res) {
          if (res.data.flag&&res.data.tipsmap) {
            _this.detail = res.data.tipsmap;
            console.log(_this.detail)
            _this.detailText = "";
            var content = _this.detail.tipContent.split('<br/>');
            if (content) {
              ...
              _this.detailText += "<p>" + detail.tipContent + "</p>";
            }
      ...
      getMore: function () {
        ...
        _this.$ajax.post('/cnews/tips/inter/getRecommentTips.do', str).then(function (res) {
