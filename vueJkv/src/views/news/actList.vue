<template>
  <div>
      ...
      <div class="content">
        <div class="search-top">
          ...
          <div class="right">
            ...
            <ul class="type-list">
              <li :class="typeCheckedId == item.cateId?'th-cli':''" v-for="(item,index) in actTypeList" :key="index"
       ...
       <!--   </div>-->
        ...
        <div class="search-panel">
          ...
          <button @click="getActivePage()"></button>
    ...
    <div class="n-list">
      <ul>
        <li class="list-3" v-for="item in pageList" :key="item">
          <div>
            <span class="1-1"></span>
            <span class="1-2"></span>
          </div>
          <router-link target="_blank" :to='{path:"/actDetail/"+item.actId}'>{{item.title}}</router-link>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getActTypeList: function () {
        let _this = this;
        _this.$ajax.post('/cnews/act/inter/getActTypeList.do').then(function (res) {
          if (res.data.flag) {
            _this.actTypeList = res.data.list;
            console.log(_this.actTypeList)
          }
        })
      },
      //获取国家列表
      ...
      getActivePage: function () {
        ...
        _this.$ajax.post('/cnews/act/inter/getActsList.do', _this.$qs.stringify(_this.pageData)).then(function (res) {

          _this.pageList = res.data.rows;
