  ...
  <div>
    ...
    <div class="bread-crumb-container">
      <div class="bread-crumb">
        ...
        》
        <router-link target="_blank" to="/preferList">政策速递</router-link>
        》
    ...
    <div class="news-view">
      <div class="content">
        <div class="left">
          <h1 class="art-type">{{detail.preferTypeNm}}<span>{{formatDate(detail.instDtm,"-")}}</span></h1>
        ...
        <div class="right">
          ...
          <ul class="art-list" v-for="item in detalList">
            <li v-if="item.preferId !=$route.params.id">
              <span></span>
              <router-link target="_blank" :to='{path:"/preferDetail/"+item.preferId}' @click="getDetail()">{{item.title}}</router-link>
            </li>
          </ul>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getDetail: function () {
        var _this = this
        var str = _this.$qs.stringify({preferId: _this.$route.params.id})
        _this.$ajax.post('/cnews/prefer/inter/getOnePrefer.do', str).then(function (res) {
          if (res.data.flag&&res.data.prefer) {
            _this.detail = res.data.prefer;
      ...
      getMore: function () {
        ...
        _this.$ajax.post('/cnews/prefer/inter/getPrefersList.do', str).then(function (res) {

          if (res.data.flag) {
            if (res.data.preferList && res.data.preferList.length > 0) {
              _this.detalList = res.data.preferList;
            }
          }
