<template>
  <div>
    ...
    <div class="n-search-container">
      <div class="content">
        <div class="search-top">
          <div class="left">提示类型</div>
          <div class="right">
            <div class="no-limit" :class="typeNo?'th-cli':''" @click="typeNoFun()">不限</div>
            <ul class="type-list">
              <li :class="typeCheckedId == item.cateId?'th-cli':''" v-for="(item,index) in typeList" :key="index"
    ...
    <div class="n-list">
      <ul>
        <li class="list-2" v-for="item in pageList">
          <div>
            <span class="1-1"></span>
            <span class="1-2"></span>
          </div>
          <router-link target="_blank" :to='{path:"/tipsDetail/"+item.tipId}'>{{item.tipTitle}}</router-link>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getTypeList: function () {
        let _this = this;
        _this.$ajax.post('/cnews/tips/inter/getTipTypeList.do').then(function (res) {
          if (res.data.flag) {
      ...
      typeNoFun: function () {
        ...
        _this.pageData.tipType = "";
        //分页查询
        this.getPage();
      },
      typeFun: function (typeId) {
        this.typeNo = false;
        this.typeCheckedId = typeId;
        this.pageData.tipType = typeId;
        //分页查询
        this.getPage();
      },
      //分页查询
      getPage: function () {
        ...
        _this.$ajax.post('/cnews/tips/inter/getTipPage.do', _this.$qs.stringify(_this.pageData)).then(function (res) {

          _this.pageList = res.data.list;
          _this.totalNum = res.data.pageTotal;
          console.log("pageList: "+_this.pageList)
          console.log("totalNum: "+_this.totalNum)
